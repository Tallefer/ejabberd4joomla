 ...
 * @since 1.5
 */
class plgAuthenticationOpenfire extends JPlugin
{
	/**
	 * Constructor
	 *
	 ...
	 * @since	1.5
	 */
	function plgAuthenticationOpenfire(& $subject, $config)
	{
		parent::__construct($subject, $config);
		$this->_config = $config;
		
		// temporary: create our openfire user table if not exists
		$db =& JFactory::getDBO();
		$query = "
	...
	function onAuthenticate( $credentials, $options, &$response )
	{
		...
		if ($response->status == JAUTHENTICATE_STATUS_SUCCESS) {
			...
			// sync the jos_users password with openfire_users password
